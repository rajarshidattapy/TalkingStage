...
test("V2 adds semantic visuals without drifting from V1 voice behavior", async (t) => {
  const [v1Text, v2Text, baselineSource, iconSource] = await Promise.all([
    ...
    readOptionalText(new URL("V2_BASELINE.md", root)),
  ...
  await t.test(
    "V2_BASELINE.md states the lock and the icon count",
    { skip: missing("V2_BASELINE.md", baselineSource) },
    () => {
