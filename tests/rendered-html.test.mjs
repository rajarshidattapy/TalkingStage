...
test("defines the TalkingStage presentation studio and metadata", async () => {
  ...
  assert.match(page, /Listens until you stop/);
  assert.match(
    css,
    /\.mic-button\.stop-button:hover:not\(:disabled\),[\s\S]*?background:\s*var\(--danger\)/,
  );
