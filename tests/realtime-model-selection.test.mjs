...
test("defaults to Realtime 2.1 while keeping Mini selectable", async () => {
  ...
  assert.match(pageSource, /gurudornaai\.realtime-model\.v2/);
  assert.match(pageSource, /X-GurudornaAI-Realtime-Model/);
  assert.match(pageSource, /role="radiogroup"/);
  assert.match(pageSource, /Voice model settings/);
  assert.match(routeSource, /request\.headers\.get\("X-GurudornaAI-Realtime-Model"\)/);
